...
* Adds two more missing input validation checks: `flametree_grow()`'s `trees` argument (previously unchecked entirely, so e.g. `trees = 2.5` silently misbehaved and other invalid input failed with cryptic low-level errors), and `flametree_save()`'s `plot` argument (previously, passing anything other than a ggplot object either failed with a cryptic error or, in the case of `plot = NULL`, silently saved an unrelated plot via `ggplot2::last_plot()` instead of erroring).
* Switches all internal code, `@examples`, `README.Rmd`, and vignettes to use the base R native pipe `|>` instead of the magrittr pipe `%>%`, and adds `Depends: R (>= 4.1.0)` accordingly. The `%>%` operator re-exported from magrittr is unaffected and still available for backward compatibility.
