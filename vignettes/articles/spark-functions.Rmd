...
The previous examples all use `spark_linear()`, but flametree provides three other spark function generators. The `spark_nothing()` generator produces a spark function that always returns zero, which is occasionally useful, whereas the `spark_random()` function injects uniform random noise. This can be useful with the "native flora" plot style:
